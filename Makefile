.PHONY: check format-check test vet verify-runtime build-windows clean
...
APP_PACKAGE_DIR := cmd/fpga-dma-multi-tool
...
check: format-check test vet verify-runtime
...
build-windows: verify-runtime
	...
	@cp "$(APP_PACKAGE_DIR)"/*.go go.mod go.sum "$(BUILD_SOURCE)/"
	@sed 's/^var version = ".*"$$/var version = "$(VERSION)"/' \
		"$(APP_PACKAGE_DIR)/main.go" > "$(BUILD_SOURCE)/main.go"
